...
def calculate_spatial_autocorrelation(image, max_distance=50):
    ...
    max_distance : int, optional
        Maximum distance to calculate autocorrelation

    Returns
    -------
    tuple
        (distances, autocorr) arrays
    """
...
def analyze_spatial_variations(image_path, row_step=10, col_step=10, patch_size=50):
    """
    Analyze spatial variations in microscopy images.
    ...
    # Calculate signal and background statistics
...
def create_spatial_visualization(results, normalized_image):
    """
    Create visualization plots for spatial analysis.

    Parameters
    ----------
    results : dict
        Results from analyze_spatial_variations
    ...
    ax5.axhline(y=1 / np.e, color='r', linestyle='--', label='1/e threshold')
    ax5.set_title(
...
def analyze_microscopy_spatial(image_path):
    """
    Main function for spatial analysis of microscopy images.
    ...
    -------
    tuple
        (results, figure) containing analysis results and visualization
    """
    # Load and normalize image
    normalized_image, _ = load_and_normalize_image(image_path)

    # Perform analysis
    results = analyze_spatial_variations(image_path)

    # Create visualization
    fig = create_spatial_visualization(results, normalized_image)
