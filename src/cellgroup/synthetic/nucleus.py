...
class Nucleus(BaseModel):
    ...
    A nucleus represents a cell nucleus with properties for position, size, orientation,
    and growth dynamics. It can grow, divide, move, and change its status over time.
    ...
    # The AfterValidator converts tuples to numpy arrays for internal calculations
    ...
    raw_int_density: Optional[float] = None
    """Raw integrated density, i.e., fluorescence intensity of the nucleus.
    Disabled if `None`."""
    ...
    def bounding_box(self) -> tuple[tuple[float, float], ...]:
        ...
        return tuple(
            (float(c - a), float(c + a)) for c, a in zip(self.centroid, self.semi_axes)
        )
    ...
    def roundness(self) -> float:
        ...
        if self.is_3D:
            ...
            return self.area / (np.pi * np.max(self.semi_axes) ** 2)
    ...
    # Removing unimplemented property
    # @property
    ...
    def die(self) -> None:
        ...
        shrink_factor = 0.5
        self.semi_axes = tuple(float(a * shrink_factor) for a in self.semi_axes)
    ...
    def divide(self) -> tuple["Nucleus", "Nucleus"]:
        ...
        **How it works**
        ...
        d1.semi_axes = tuple(float(s * scale_factor) for s in self.semi_axes)
        d2.semi_axes = tuple(float(s * scale_factor) for s in self.semi_axes)

        # --- Divide intensity roughly equally (with some noise)
        if self.raw_int_density is not None:
            intensity_ratio = np.random.normal(0.5, 0.05)
            d1.raw_int_density = self.raw_int_density * intensity_ratio
            d2.raw_int_density = self.raw_int_density * (1 - intensity_ratio)

        # --- Calculate new centroids position
        longest_axis_idx = np.argmax(self.semi_axes)
        longest_axis = self.semi_axes[longest_axis_idx]

        # Create offset vectors along the longest axis
        offset = np.zeros(len(self.centroid))
        offset[longest_axis_idx] = longest_axis

        # Rotate offsets and add to mother centroid
        rot_matrix = self._get_rotation_matrix()
        offset_rotated = np.dot(rot_matrix, offset)

        # Convert back to tuples for Pydantic model
        d1.centroid = tuple(float(c + o) for c, o in zip(self.centroid, offset_rotated))
        d2.centroid = tuple(float(c - o) for c, o in zip(self.centroid, offset_rotated))
    ...
    def move(self) -> None:
        ...
        space_size = np.array(self.space.size)
        new_pos = np.minimum(
            self.centroid + displacements,
            space_size - margin
        )
        new_pos = np.maximum(new_pos, margin)

        # Update centroid (convert to tuple of floats for Pydantic model)
        self.centroid = tuple(float(x) for x in new_pos)

    def _update_angle(self, angle: float) -> float:
    ...
    def update(self) -> Status:
        ...
        self.semi_axes = tuple(float(a * np.sqrt(growth_factor)) for a in self.semi_axes)
    ...
    def _compute_ellipsoidal_distances(self, space_shape=None) -> NDArray:
        """Calculate distances from nucleus centroid in the ellipsoidal space.

        Parameters
        ----------
        space_shape : tuple[int, ...], optional
            Shape of the space in which to compute distances. If None, use the nucleus's
            space size.
        ...
        distances : NDArray
            ...
            coordinate grid.
        """
        # Use nucleus's space.size if space_shape not provided
        if space_shape is None:
            space_shape = self.space.size
        ...
        # Center coordinates
        centroid_array = np.asarray(self.centroid)
        coords = coords - centroid_array[:, None]

        # Rotate coordinates
        coords = np.dot(self._get_rotation_matrix(), coords)

        # Normalize coordinates
        semi_axes_array = np.asarray(self.semi_axes)
        coords = coords / semi_axes_array[:, None]
    ...
    def render(self) -> NDArray:
        """Render the nucleus as a binary mask in its space.
